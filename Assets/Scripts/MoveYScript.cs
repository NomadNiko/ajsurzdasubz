...
public class MoveYScript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            transform.Translate(Vector3.right * Time.deltaTime * (speedKeeper.speed + 4));
        }
        else        // Move object backward
        {
            transform.Translate(Vector3.right * Time.deltaTime * speedKeeper.speed);
        }
        ...
        if (transform.position.y < zBound && gameObject.CompareTag("Obstacle"))
        {
            Destroy(gameObject);
        }
    }
